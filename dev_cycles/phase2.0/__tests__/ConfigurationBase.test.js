...
describe('ConfigurationBase file i/o:', () => {
    ...
    let config = new ConfigurationBase({version: '0.2.0',
        config_file: './__tests__/input/ConfigurationBase.yml',
    ...
    let output_config_file = './__tests__/output/ConfigurationBase.yml';
...
describe('error conditions', () => {
	...
	test('dumpToYAML() failure (unable to write file)', () => {
		let config = new ConfigurationBase({root_title:'DomainTitle',
			title:'SectionTitle'});
		expect(() => { config.dumpToYAML({file: './__tests/input/READONLY'})
		}).toThrow();
	});
});
