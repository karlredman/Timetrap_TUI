...
DetailsTable.prototype.registerActions = function() {
  ...
  this.view.timetrap.on('command_complete', (emit_obj) => {
    if (emit_obj.owner === 'details_table_now') {
      ...
      // find sheet
      const arr = emit_obj.data.stdoutData.split('\n');
      ...
      if (arr.length > 0) {
        for (let i = 0; i < arr.length; i++) {
          // sheet portion
          const chunk = arr[i].slice(1, arr[i].length);
          const sheet = chunk.split(':')[0];

          if (sheet === _this.view.sheet) {
            // figure out the current durration
            // let d1 = arr[i].split(' ')
            const d1 = chunk.split(' ');
            duration = d1[1];
            ...
            const start_time = helpers.zeropad(n.getHours())
            ...
            const start_date = days[n.getDay()] + ' '
                            + helpers.zeropad(months[n.getMonth()]) + ' '
                            + helpers.zeropad(n.getDate())
                            + ', ' + n.getFullYear();

            // append to table
            const rec = [id, start_date, start_time, '--------', duration, note];
    ...
    if (emit_obj.owner === 'details_table_ids') {
      if (emit_obj.data.type === 'ids') {
        ...
        const id_ar = emit_obj.data.stdoutData.split(' ');
    ...
    if (emit_obj.owner === 'detailstable') {
      if (
        ...
        if (typeof emit_obj.data.stdoutData !== 'undefined') {
          ...
          //
          const content = JSON.parse(String(emit_obj.data.stdoutData));
  ...
  _this.rows.on('mouse', function(ch, key) {
    const idx = this.getItemIndex(this.selected);
    this.select(idx);
  });
