...
Menubar.prototype.registerActions = function() {
  ...
  this.on('prompt', function(data) {
    if (
      ...
      const selected = _this.view.widgets.sheettree.rows.selected;
      const node_lines = _this.view.widgets.sheettree.nodeLines;
      const sheet = node_lines[selected].sheet;
...
Menubar.prototype.init = function() {
  ...
  const items = {
    ...
    'In': () => {
      ...
      const dlg = new Prompt({widget: _this}).cannedInput('checkIn');
    },
    // 2
    'Out': () => {
      const selected = _this.view.widgets.sheettree.rows.selected;
      ...
      const dlg = new Prompt({widget: _this}).cannedInput('checkOut');
    },
    // 3
    'Edit': () => {
      const selected = _this.view.widgets.sheettree.rows.selected;
      ...
      const dlg = new Prompt({widget: _this}).cannedInput('edit');
    },
    // 4
    'Task': () => {
      const selected = _this.view.widgets.sheettree.rows.selected;
      const idx = _this.view.widgets.sheettree.rows.getItemIndex(_this.selected);
      const node_lines = _this.view.widgets.sheettree.nodeLines;
      const sheet = node_lines[selected].sheet;
      ...
      if (_this.view.widgets.sheettree.nodeLines[selected].info.running !== '0:00:00') {
        ...
        // it's not running
        // // checkin
        const dlg = new Prompt({widget: _this}).cannedInput('task');
    ...
    'Details': () => {
      ...
      const sheet = node_lines[selected].sheet;
      const id = node_lines[selected].info.id;
      const note = node_lines[selected].info.note;
    ...
    'Stop all': () => {
      if (_this.view.widgets.sheettree.num_running > 0) {
        if (_this.view.process_config.data.question_prompts.value === true) {
          const dlg = new Question({widget: _this}).cannedInput('stopAll');
        } else {
