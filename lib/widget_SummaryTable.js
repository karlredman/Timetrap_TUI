...
SummaryTable.prototype.updateSummaryTimes = function() {
  ...
  const delta = now - this.fake_timer_time;
...
SummaryTable.prototype.updateSummaryData = function() {
  const node_lines = this.view.widgets.sheettree.nodeLines; // data in sidebar tree
  ...
  const idx = this.view.widgets.sheettree.rows.getItemIndex(this.view.widgets.sheettree.rows.selected);
  this.rows.select(idx);
