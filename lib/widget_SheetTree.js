...
var ContribTree = require('./blessed-contrib/tree');
...
SheetTree.prototype.processList = function(result) {
  ...
  for (; i < arr.length; i++) {
    ...
    const darr = arr[i].substr(1, arr[i].length).match(/\S+/g);
    if (darr != null) {
      j.name = darr[0];
      j.running = darr[1];
      j.today = darr[2];
...
SheetTree.prototype.buildTree = function(list) {
  ...
  for (let i = 0; i < input.length; i++) {
    const chain = input[i].name.split('.');
    ...
    for (let j = 0; j < chain.length; j++) {
      ...
      if (lastNode == currentNode) {
        ...
        if (typeof _this.running_data !== 'undefined') {
          for (const r in _this.running_data) {
        ...
        const newNode = currentNode[k] = {name: wantedNode, extended: true, sheet: sheet_val, info: list_info_val, children: []};
  ...
  const tree = {name: 'Timetrap', extended: true, sheet: 'default',
    info: {
      running: '-:--:--',
      today: '-:--:--',
      total_time: '-:--:--',
